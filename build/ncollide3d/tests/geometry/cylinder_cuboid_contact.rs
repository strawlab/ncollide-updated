...
fn cylinder_cuboid_contact() {
    ...
    let distance = query::distance_support_map_support_map(&cyl_at, &cyl, &cuboid_at, &cuboid);

    let proximity =
        query::proximity_support_map_support_map(&cyl_at, &cyl, &cuboid_at, &cuboid, 0.1);

    let contact = query::contact_support_map_support_map(&cyl_at, &cyl, &cuboid_at, &cuboid, 10.0);
